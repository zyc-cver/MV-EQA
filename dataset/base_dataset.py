...
class _MixamoDatasetBase(Dataset):
    def __init__(self, phase, config):
        ...
        if phase == 'train':
            self.character_names = ['Aj', 'BigVegas', 'Claire', 'Lola', 'Malcolm', 'Jasper',
                                    'Warrok', 'Globin', 'Kaya', 'PeanutMan', 'Pearl']
            self.aug = True
        else:
            self.character_names = ['Ty', 'Andromeda',
                                    'Pumpkinhulk', 'SportyGranny']
            self.aug = False

        items = glob.glob(os.path.join(
            self.data_root, self.character_names[0], '*/motions/*.npy'))
    ...
    def preprocessing(self, item, view_angle=None, param=None):
        ...
        motion_proj = trans_motion3d(motion3d, local3d, self.unit)  # new
        motion_proj = normalize_motion(
            motion_proj, self.mean_pose, self.std_pose)
        # angles = calculate_angles(motion_proj)
        # reshape to (joints*2, len_frames)
        motion_proj = motion_proj.reshape((-1, motion_proj.shape[-1]))
        # motion_proj = np.concatenate((motion_proj, angles), axis=0)  # new
        # FIXME : change to torch.from_numpy?
        motion_proj = torch.Tensor(motion_proj)
        return motion_proj

    def preprocessing_exp(self, motion3d, view_angle=None, param=None):
        """
        :param item: filename built from self.build_tiem
        :return:
        """
...
def gen_meanpose(config):
    all_paths = glob.glob(os.path.join(
        config.data_dir, 'train', '*/*/motions/*.npy'))
    all_joints = []
