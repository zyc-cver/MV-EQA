...
def two_pts_to_rectangle(point1, point2):
    ...
    if beta <= -180:
        beta += 360
    p1 = (int(point1[0] - length*math.cos(math.radians(beta))),
          int(point1[1] - length*math.sin(math.radians(beta))))
    p2 = (int(point1[0] + length*math.cos(math.radians(beta))),
          int(point1[1] + length*math.sin(math.radians(beta))))
    p3 = (int(point2[0] + length*math.cos(math.radians(beta))),
          int(point2[1] + length*math.sin(math.radians(beta))))
    p4 = (int(point2[0] - length*math.cos(math.radians(beta))),
          int(point2[1] - length*math.sin(math.radians(beta))))
    return [p1, p2, p3, p4]
...
def joints2image_dual(motion1, motion2, color1, color2, transparency=False, H=512, W=512, imtype=np.uint8):
    nr_joints = 15  # 固定关节数量为15

    limbSeq = [[0, 1], [1, 2], [1, 5], [1, 8], [2, 3], [3, 4], [5, 6], [6, 7],
               [8, 9], [8, 12], [9, 10], [10, 11], [12, 13], [13, 14]]

    # 设置颜色
    def set_colors(colors, transparency):
        L = rgb2rgba(colors[0]) if transparency else colors[0]
        M = rgb2rgba(colors[1]) if transparency else colors[1]
        R = rgb2rgba(colors[2]) if transparency else colors[2]
        return M, L, R

    M1, L1, R1 = set_colors(color1, transparency)
    M2, L2, R2 = set_colors(color2, transparency)

    colors_joints1 = [M1, M1, L1, L1, L1, R1, R1, R1, M1, L1, L1, L1, R1, R1, R1]
    colors_joints2 = [M2, M2, L2, L2, L2, R2, R2, R2, M2, L2, L2, L2, R2, R2, R2]

    colors_limbs = [M1, L1, R1, M1, L1, L1, R1, R1, L1, R1, L1, L1, R1, R1]  # 用于绘制肢体

    if transparency:
        canvas = np.zeros(shape=(H, W, 4), dtype=imtype)
    else:
        canvas = np.ones(shape=(H, W, 3), dtype=imtype) * 255

    # 绘制函数，用于绘制单个人物
    def draw_figure(joints_position, colors_joints, colors_limbs, canvas):
        hips = joints_position[8]
        neck = joints_position[1]
        torso_length = ((hips[1] - neck[1]) ** 2 + (hips[0] - neck[0]) ** 2) ** 0.5
        ...
        for i in range(1, len(colors_joints)):
            cv2.circle(canvas, (int(joints_position[i][0]), int(joints_position[i][1])), joints_radius, colors_joints[i], thickness=-1)

        for i in range(len(limbSeq)):
            limb = limbSeq[i]
            cur_canvas = canvas.copy()
            point1 = joints_position[limb[0]]
            point2 = joints_position[limb[1]]
            X = [point1[1], point2[1]]
            Y = [point1[0], point2[0]]
            mX = np.mean(X)
            mY = np.mean(Y)
    ...
    # 绘制两个人物
    canvas = draw_figure(motion1, colors_joints1, colors_limbs, canvas)
    canvas = draw_figure(motion2, colors_joints2, colors_limbs, canvas)
...
def motion2video_dual(motion1, motion2, h, w, save_path, colors1, colors2, transparency=False, fps=25, save_frame=False):
    nr_joints = 15
    videowriter = imageio.get_writer(save_path, fps=fps)
    vlen = min(motion1.shape[-1], motion2.shape[-1])  # 取两个动作中较短的长度
    if save_frame:
        frames_dir = save_path[:-4] + '-frames'
        ensure_dir(frames_dir)
    for i in tqdm(range(vlen)):
        ...
        # 绘制第一个动作
        [img1, _] = joints2image_dual(motion1[:, :, i], motion2[:, :, i],
                                 colors1, colors2, transparency, H=h, W=w)
        canvas = cv2.addWeighted(canvas, 0, img1, 1, 0)

        if save_frame:
            save_image(canvas, os.path.join(frames_dir, "%04d.png" % i))

        videowriter.append_data(canvas)
    videowriter.close()


def motion2video_dual_(motion1, motion2, h, w, save_path, colors1, colors2, transparency=False, motion_tgt=None, fps=25, save_frame=False):
    nr_joints = motion1.shape[0]
    videowriter = imageio.get_writer(save_path, fps=fps)
    ...
    for i in tqdm(range(vlen)):
        [img, img_cropped] = joints2image(
            motion1[:, :, i], colors1, motion2[:, :, i], colors2, transparency, H=h, W=w, nr_joints=nr_joints)
        if motion_tgt is not None:
            [img_tgt, img_tgt_cropped] = joints2image(
                motion_tgt[:, :, i], colors1, transparency, H=h, W=w, nr_joints=nr_joints)
            img_ori = img.copy()
            img = cv2.addWeighted(img_tgt, 0.3, img_ori, 0.7, 0)
            img_cropped = cv2.addWeighted(img_tgt, 0.3, img_ori, 0.7, 0)
            bb = bounding_box(img_cropped)
    ...
    if _circle:
        for i in range(len(joint_colors)):
            cv2.circle(canvas, (int(all_peaks[i][0]), int(
                all_peaks[i][1])), 2, joint_colors[i], thickness=2)
