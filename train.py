...
def main():
    ...
    parser.add_argument('-g', '--gpu_ids', type=int, default=0,
                        required=False, help="specify gpu ids")
    ...
    val_loader = get_dataloader(
        'test', config, config.batch_size, config.num_workers)
    ...
    for e in range(config.nr_epochs):
        ...
        for b, data in enumerate(pbar):
            ...
            pbar.set_description(
                "EPOCH[{}][{}/{}]".format(e, b, len(train_loader)))
            pbar.set_postfix(OrderedDict(
                {"loss": sum(losses_values.values())}))
